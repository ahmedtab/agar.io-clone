...
var socket;
var reason;
var gamePath = window.location.pathname.replace(/\//g, "").replace(/.aik/g, "");
...
var login = addEventListener('enter', this.gameInput, false);
...
function startGame(type) {
    ...
    if (!socket) {
        socket = io({ query: "type=" + type, path: "/"+gamePath });
        setupSocket(socket);
    }
    ...
    socket.on("leaderboard", function (data) {
        ...
        for (var i = 0; i < leaderboard.length; i++) {
            ...
            if (leaderboard[i].id == player.id) {
                ...
                if (leaderboard[i].name.length !== 0)
                    status += (leaderboard[i].mass.toFixed(0)) + ". " + leaderboard[i].name;
                else status += (leaderboard[i].mass.toFixed(0)) + ". An unnamed cell";
            }
        }
...
function drawVirus(virus) {
    ...
    Ximage.src = "img/logo.png";

    var imgWidth = 93;
    var imgHight = 125;
    graph.drawImage(
        Ximage,
        (virus.x - player.x + global.screenWidth / 2 ) - (imgWidth/2),
        (virus.y - player.y + global.screenHeight / 2) - (imgHight/2),
        imgWidth,
        imgHight
    );
}
...
function drawborder() {
    ...
    if (player.y <= global.screenHeight / 2) {
        graph.beginPath();
        graph.moveTo(
            0 ? player.x > global.screenWidth / 2: global.screenWidth / 2 - player.x,
