...
exports.StartGameServer = function (io) {
    ...
    function addFood(toAdd) {
        ...
        while (toAdd--) {
            ...
            if (c.foodUniformDisposition)
                position = util.uniformPosition(food, radius);
            else position = util.randomPosition(radius);
    ...
    function addVirus(toAdd) {
        while (toAdd--) {
            ...
            var position;
            if (c.virusUniformDisposition)
                position = util.uniformPosition(virus, radius);
            else position = util.randomPosition(radius);
            virus.push({
    ...
    function movePlayer(player) {
        ...
        for (var i = 0; i < player.cells.length; i++) {
            ...
            if (player.cells[i].speed <= 6.25) {
                slowDown =
                    util.log(player.cells[i].mass, c.slowBase) -
                    initMassLog +
                    1;
            }
            ...
            for (i = 0; i < users.length; i++) {
                for (var z = 0; z < users[i].cells.length; z++) {
                    if (
                        ...
                        var massLoss =
                            users[i].cells[z].mass *
                            (1 - c.massLossRate / 1000);
